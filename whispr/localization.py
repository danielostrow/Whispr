...
class SpatialFeatureExtractor:
    ...
    def __init__(self, sample_rate=16000, n_fft=1024, hop_length=512):
        ...
        try:
            ...
            log.warning(
                "Librosa not available. Spatial feature extraction will be limited."
            )
    ...
    def _extract_multichannel_features(self, audio):
        ...
        for i in range(num_channels):
            for j in range(i + 1, num_channels):
                ...
                stft_i = librosa.stft(
                    audio[i], n_fft=self.n_fft, hop_length=self.hop_length
                )
                stft_j = librosa.stft(
                    audio[j], n_fft=self.n_fft, hop_length=self.hop_length
                )
        ...
        features = np.concatenate(
            [np.concatenate(channel_features), np.array(spatial_features)]
        )
...
def load_localization_model(model_path=None):
    ...
    if model_path is None:
        paths_to_check = [
            ...
            os.path.join(
                os.path.dirname(__file__), "../models/whispr_localization_model.pth"
            ),
    ...
    feature_params_path = os.path.join(
        os.path.dirname(model_path), "feature_params.json"
    )
...
def assign_locations(speakers: List[dict]):
    ...
    log.info(
        f"Estimated room dimensions: {room_width}m x {room_length}m x {room_height}m"
    )
    ...
    if model is None or feature_extractor is None:
        ...
        if multi_channel_audio is not None:
            ...
            _assign_locations_by_triangulation(
                speakers, multi_channel_audio, room_width, room_length, room_height
            )
        else:
...
def _assign_locations_in_circle(
    speakers: List[dict], room_width: float, room_length: float
):
...
def _assign_locations_by_triangulation(
    speakers: List[dict],
